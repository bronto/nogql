    ...
    switch (url[1]) {
      ...
      case 'list':
        ...
        if (req.method === 'GET') {
          ...
          query('SELECT id, config_id, host_name, address, notes FROM tbl_host WHERE ? = ?', [search_flagname,search_flag]).then(function (results) {
            if (results[0].length != 1) {
              sendError(404, 'could not retrieve configuration for requested state');
            }
            result  = results[0][0];
            logger.info('Found hosts:' + results);
            if (req.method === 'GET') {
              sendResponse(200, result);
              return;
            }
