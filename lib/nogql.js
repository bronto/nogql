    ...
    switch (url[1]) {
      ...
      case 'list':
        ...
        if (req.method === 'GET') {
          ...
          switch (qlstatus) {
            case 'active':
              search_filter = "WHERE active='1'";
              break;
            case 'deactive':
              search_filter = "WHERE active='0'";
              break;
            case 'registered':
              search_filter = "WHERE register='1'";
              break;
            default:
              sendError(400, 'request does not contain a valid status(active/registered)');
              return;
          }
          query('SELECT id, host_name, address FROM tbl_host ?', [search_filter]).then(function (results) {
            if (results[0].length != 1) {
              sendError(404, 'could not retrieve configuration for requested state');
            }
            console.log(results);
            for ( var result of results[0] ) {
              logger.info('Found hosts:' + result.host_name);
            };
            if (req.method === 'GET') {
